...
$\begin{cases}\min f(\mathbf{x}) -\mu \sum_{i=1}^m\log(z_i)\\
\text{s.t.}\quad \mathbf{c}(\mathbf{x}) - \mathbf{z} = \mathbf{0}
\end{cases}$
"""
...
md"""
And the KKT conditions can be written as
...
where $J_\mathbf{c}(\mathbf{x})$ is the Jacobian of $\mathbf{c}(\mathbf{x})$, while

$Z = \begin{bmatrix} z_1 & & \\ & \ddots & \\ & & z_m\end{bmatrix}
\quad 
...
md"""
The KKT equations can be group together and written in the form

$F = F\begin{pmatrix}\mathbf{x}\\ \boldsymbol{\lambda}\\ \mathbf{z}\end{pmatrix} = 
\begin{pmatrix}
\nabla\!_x \mathcal{L} \\
...
md"""
This system of equations can be solved iteratively using the Newton's method:

$\begin{pmatrix}\mathbf{x}^{(k+1)}\\ \boldsymbol{\lambda}^{(k+1)}\\ \mathbf{z}^{(k+1)}\end{pmatrix}
=
\begin{pmatrix}\mathbf{x}^{(k)}\\ \boldsymbol{\lambda}^{(k)}\\ \mathbf{z}^{(k)}\end{pmatrix}
+ \alpha_k
\begin{pmatrix}\delta\mathbf{x}^{(k)}\\ \delta\boldsymbol{\lambda}^{(k)}\\ \delta\mathbf{z}^{(k)}\end{pmatrix}$
"""

# ╔═╡ d7c8ab1b-e81d-4089-889d-5524a9d200ab
md"""
Where the term $(\delta\mathbf{x}^{(k)}, \delta\boldsymbol{\lambda}^{(k)}, \delta\mathbf{z}^{(k)})^T$ it the solution of the system:

$J_F^{(k)}\begin{pmatrix}\delta\mathbf{x}^{(k)}\\ \delta\boldsymbol{\lambda}^{(k)}\\ \delta\mathbf{z}^{(k)}\end{pmatrix}
=
F^{(k)}$

with $F^{(k)}$ being $F$ evaluated in the $k$-th step

$F^{(k)}
=
F\begin{pmatrix}\mathbf{x}^{(k)}\\ \boldsymbol{\lambda}^{(k)}\\ \mathbf{z}^{(k)}\end{pmatrix}$
...
md"""
To check if this is indeed the case, we use the Bunch-Kaufman factorization of a symmetric indefinite matrix $S$ into a (lower) triangular matrix $L$ and in a block diagonal one, $B$.
...
function inertia(b::BunchKaufman)
	...
	zeigs  = 0
	for i in 1:N
		# check if it is a 2x2 block
		if ld[i+1] ≠ 0 || ud[i+1] ≠ 0
			continue
		elseif ld[i] ≠ 0 || ud[i] ≠ 0
			peigs += 1
			meigs += 1
		else
			if md[i] > 0
...
# ╔═╡ 68ffe555-d47a-462d-af54-347a6568d11a
md"""
where $\nu$ is another barrier parameter.
"""

# ╔═╡ f14ba32d-fafe-4f27-80a5-31cb9ff753cf
md"""
To enforce that each step will be inside the feasible set, want to find a value $\alpha^{(k)} \in (0, \tilde{\alpha}_z^{(k)}]$ such that
...
# ╔═╡ e204a9af-2d5d-4d13-9fd9-ef29c300568a
md"""
...
function update_mu(μ, xₖ, zₖ, λₖ, ∇fₖ, ∇²fₖ, cₖ, Jcₖ; σ=0.2)
...
md"""
Now we have all the pieces to solve numerically our problems.
"""

# ╔═╡ 6a6b6045-f8ed-4a39-a0e6-b7d1eda6ed0f
md"""
### Interior point constrained optimization
"""
...
function optimize(f, ∇f, ∇²f, c, Jc, ∇²c, x₀; μ₀=1e-2, ν₀=1e-1, δ₀=0, tol=1e-7, maxitr=100, linesearch=true)
	...
	while k ≤ maxitr && !converged
		...
		push!(values, f(xₖ))
		# Convergence check
		converged = error(xₖ, zₖ, λₖ, ∇fₖ, ∇²fₖ, cₖ, Jcₖ, 0) < tol
	end
